...
import { getEducationProviders } from 'apiSdk/education-providers';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['manager', 'content-creator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<EducationProviderInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/education-providers` : null),
    () => getEducationProviders({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/education-providers/view/${data?.[0]?.id}` : '/education-providers',
