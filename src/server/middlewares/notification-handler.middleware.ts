...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'content.create': {
    roles: ['manager'],
    key: 'content-created',
    tenantPath: ['education_provider', 'content'],
    userPath: [],
  },
  'content.update': {
    roles: ['manager'],
  ...
  'student.update': {
    roles: ['manager', 'content-creator'],
    key: 'student-progress-updated',
    tenantPath: ['education_provider', 'course', 'student'],
    userPath: [],
  },
  'student.delete': {
    roles: ['manager'],
    key: 'student-profile-deleted',
    tenantPath: ['education_provider', 'course', 'student'],
    userPath: [],
  },
};
...
const tenantRoles: string[] = ['manager', 'content-creator'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.education_provider.findFirst({
