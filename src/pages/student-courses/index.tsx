...
import { getStudentCourses, deleteStudentCourseById } from 'apiSdk/student-courses';
import { StudentCourseInterface } from 'interfaces/student-course';

type ColumnType = ColumnDef<StudentCourseInterface, unknown>;

interface StudentCourseListPageProps {
...
export function StudentCourseListPage(props: StudentCourseListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<StudentCourseInterface>>(
    () => `/student-courses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: StudentCourseInterface) => {
    if (hasAccess('student_course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/student-courses/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/student-courses/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('student_course', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/student-courses/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="student_course" />
        </Flex>

        {hasAccess('student_course', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/student-courses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
