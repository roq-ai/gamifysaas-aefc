...
export function CourseListPage(props: CourseListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getCourses({
        relations: ['education_provider', 'content', 'student.count', 'student_course.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('education_provider', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'education_provider',
          header: 'Education Provider',
          accessorKey: 'education_provider',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/education-providers/view/${record.education_provider?.id}`}
            >
              {record.education_provider?.name}
            </Link>
          ),
        }
      : null,
    ...
    hasAccess('student_course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.student_course,
        }
      : null,
