...
import { StudentCourseListPage } from 'pages/student-courses';
...
function CourseViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CourseInterface>(
    ...
    () =>
      getCourseById(id, {
        relations: ['education_provider', 'content'],
      }),
  );
              ...
              >
                ...
                {hasAccess('education_provider', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    ...
                    text={
                      <Link as={NextLink} href={`/education-providers/view/${data?.education_provider?.id}`}>
                        {data?.education_provider?.name}
                      </Link>
                    }
                  />
                )}
