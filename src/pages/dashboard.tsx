...
import { getStudentCourses } from 'apiSdk/student-courses';
...
function DashboardPage() {
  ...
  const studentCourseStat = useSWR<PaginatedInterface<StudentCourseInterface>>(
    () => `/student-courses`,
    () => getStudentCourses({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: studentCourseStat?.data?.totalCount,
      isLoading: studentCourseStat.isLoading,
